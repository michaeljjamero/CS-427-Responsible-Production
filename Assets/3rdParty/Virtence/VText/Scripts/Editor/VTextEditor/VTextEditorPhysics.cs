...
public class VTextEditorPhysics : AbstractVTextEditorComponent
{
    ...
    private SerializedProperty _physics;                                                // the physics component of this vtext object
    private SerializedProperty _createRigidbody;                                        // create a rigidbody?
    private SerializedProperty _rigidbodyMass;                                          // the mass of the rigidbody
    private SerializedProperty _rigidbodyDrag;                                          // the drag value of the rigidbody
    private SerializedProperty _rigidbodyAngularDrag;                                   // the angular drag component of the rigidbody
    private SerializedProperty _rigidbodyGravity;                                       // use gravity?
    private SerializedProperty _rigidbodyIsKinematic;                                   // determines if the rigidbody is kinematic or not

    #endregion RIGIDBODY

    #region COLLIDER

    private SerializedProperty _colliderIsTrigger;                                      // determines if the collider is setup as a trigger
    private SerializedProperty _colliderIsConvex;                                       // determines if the collider is convex (only use for MeshCollider!!!)

    private SerializedProperty _colliderType;                                           // the collider type (none, box, mesh-collider, etc)
    private SerializedProperty _colliderMaterial;                                       // the physics material for this collider
    ...
    public VTextEditorPhysics(SerializedObject obj, UnityEditor.Editor currentEditor)
    {
        _physics = obj.FindProperty("PhysicsParameter");
    ...
    private bool ShowColliderEditor()
    {
        ...
        int colliderTypeID = (int)(VTextPhysicsParameter.ColliderType)EditorGUILayout.EnumPopup(new GUIContent("Collider type:", "The type of the collider added to each letter"),
            (VTextPhysicsParameter.ColliderType)System.Enum.GetValues(typeof(VTextPhysicsParameter.ColliderType)).GetValue(_colliderType.enumValueIndex));
        ...
        if ((VTextPhysicsParameter.ColliderType)_colliderType.enumValueIndex != VTextPhysicsParameter.ColliderType.None && _showCollider.value == false)
        {
            _showCollider.target = true;
        }
        else if ((VTextPhysicsParameter.ColliderType)_colliderType.enumValueIndex == VTextPhysicsParameter.ColliderType.None && _showCollider.value == true)
        {
        ...
        if (EditorGUILayout.BeginFadeGroup(_showCollider.faded))
        {
            ...
            if ((VTextPhysicsParameter.ColliderType)_colliderType.enumValueIndex == VTextPhysicsParameter.ColliderType.Box)
            {
                updatePhysics |= ShowBoxColliderParameters();
            }
            else if ((VTextPhysicsParameter.ColliderType)_colliderType.enumValueIndex == VTextPhysicsParameter.ColliderType.Mesh)
            {
                updatePhysics |= ShowMeshColliderParameters();
            }

            // physics material ... this will work for all colliders
            if ((VTextPhysicsParameter.ColliderType)_colliderType.enumValueIndex != VTextPhysicsParameter.ColliderType.None)
            {
                GUILayout.BeginHorizontal();
